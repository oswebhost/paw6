...
<table border="0" cellpadding="2" cellspacing="0" style="border:1px solid #ccc;margin:auto auto;" bordercolor="#f4f4f4" width="567">
	  <form method="get" action="<?php echo  $PHP_SELF;?>">
        ...
        while ($sr = $temp->fetch()) {
		  ...
		  <?php for ($i=47; $i>=1; $i--) { ?>
			  <option value="<?php echo  $i;?>" <?php if($i==$cur): echo " selected"; endif;?>>
            &nbsp;<?php echo  $i;?>&nbsp;&nbsp;&nbsp;
        </option>
		  <?php } ?>		 
             ...
             <?php if ($db=='eu'){ ?>
    			...
    			<?php for($_i=0; $_i<count($arry_div); $_i++){ ?>
    			  <?php if($_i<>4 and $_i<>9 and $_i<>18){ ?>
    					<option value="<?php echo  $arry_div[$_i];?>" <?php echo  selected($_GET['DIV'], $arry_div[$_i]);?>>
                    <?php echo  divname($arry_div[$_i]); ?>
                </option>
    			   <?php } ?>
    			<?php } ?>
             <?php } ?>
...
<tr <?php echo  rowcol($number);?>>
    ...
    <td class="ctd" <?php echo  ($_GET["SORTBY"]=='1'? "bgcolor='#D3EBAB'": "");?>><?php echo  ($row["dcr_ht"]); ?></td>
    <td class="ctd" <?php echo  ($_GET["SORTBY"]=='2'? "bgcolor='#D3EBAB'": "");?>><?php echo  ($row["dcr_at"]); ?></td>
    <td class="ctd" <?php echo  ($_GET["SORTBY"]=='4'? "bgcolor='#D3EBAB'": "");?>><?php echo  ($row["dcr_av"]); ?></td>
    <td class="ctd"><?php echo  ($row["dcrdif"]); ?></td>
