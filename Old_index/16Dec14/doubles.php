...
$qry = "SELECT f.hodddif, f.aodddif, f.dodddif, concat(f.hgoal,f.agoal) as asl2show, f.prvalue, c.dcr_ht, c.dcr_at, c.dcr_av,c.dcr_dif,abs(c.dcr_dif) as dcrdif,f.`div`, f.hteam,f.ateam,f.match_time,f.hgoal,f.agoal,f.h_s,f.a_s,f.gotit,f.mvalue,f.mid,f.pawrank, date_format(f.match_date,'%d-%b-%y') as mdate,
	...
	switch ($_GET['PROPTION'])
      ...
      { 
		...
		case 6: $call = " and f.hodddif >= $odd_maxdiff "; $row3cap = "Home Win Calls (Bookies)"; break;
		case 7: $call = " and f.aodddif >= $odd_maxdiff "; $row3cap = "Away Win Calls (Bookies)"; break;
		case 8: $call = " and ( (f.aodddif>0 and f.aodddif <= $odd_maxdiff) or (f.hodddif>0 and f.hodddif <= $odd_maxdiff) ) "; 
			$row3cap = "Draw Calls (Bookies)"; break;
      }
            ...
            case 1: //1x2 betting
               switch ($_GET['CALL'])
               {
					...
					case 8:
                        switch ($_GET['CALLPARAM'])
                        {
                            case 1: $callpar2 = " where hswin between 0 and ". $odd_maxdiff . " or aswin between 0 and " . $odd_maxdiff  ; break;
                            case 2: $callpar2 = " where hswin > ". $odd_maxdiff . " or aswin > " . $odd_maxdiff  ; break;
                        }
                        break;
               }
               break;    
   ...
   // if ($weekno==9) echo "<pre>" . print_r($_GET) . "</pre>";
   ...
   if ($db=='eu'){
	...
	while ($rows = $temp->fetch()){
	    ...
	    if (($_GET['CALL']==6) and ( $rows['h_s'] > $rows['a_s']) ){
			$nGotSql = " , gotit = '1' ";
		}
		
		if (($_GET['CALL']==7) and ( $rows['a_s'] > $rows['h_s']) ){
			$nGotSql = " , gotit = '1' ";
		}
		
		if (($_GET['CALL']==8) and ( $rows['h_s'] == $rows['a_s']) ){
			$nGotSql = " , gotit = '1' ";
		}
