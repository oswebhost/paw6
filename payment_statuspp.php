...
require_once("reguser.php");
	...
	define("USE_SANDBOX", 0);  // Set to 0 once you're ready to go live
	...
	if (strcmp ($res, "VERIFIED") == 0) {
		...
		$sk_transid       = $_POST['txn_id'];
		$merchant_id	  = "";	
		$mb_amount		  = $_POST['mc_gross'];
		$mb_currency	  = $_POST['mc_currency'];
		$pay_status		  = $_POST['payment_status'];
		$payment_type     = "PayPal";
		$sk_custid        = $_POST['payer_email'];
		$md5sig           = "** PayPal ** N/A";

		$paw_id         = $custom_values[0];// for paw only
		$paw_user_email = $custom_values[1];// for paw only
		$service_id       = $_POST['item_number'];
		
		$receiver_email   = $_POST['receiver_email'];
		
		$send_amount   = num2($charge_amt[$item_number]);
		$send_currency = "GBP";		
		
		$expire_date = calculate_expiration_date($item_number); 
		...
		(now(),'$paw_id','$service_id', '$send_amount','$mb_currency','$mb_amount',
		'$payment_type','$pay_status','$sk_transid','$sk_custid', '$md5sig')";
	...
	// echo $sql ;
	 ...
	 if ($mb_amount>0){ // update dates if send amount is >0, amount < 0 means REFUND.
