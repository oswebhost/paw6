...
// Admin edit any customer address end
      ...
      if ($error == false) {
        ...
        // Admin edit any customer address begin
//        tep_redirect(tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $customers_id));
// stay in edition view after editing data
...
// Admin edit any customer address end
      ...
      default:
        ...
        // Admin edit any customer address begin
//        $customers_query = tep_db_query("select c.customers_id, c.customers_gender, c.customers_firstname, c.customers_lastname, c.customers_dob, c.customers_email_address, a.entry_company, a.entry_street_address, a.entry_suburb, a.entry_postcode, a.entry_city, a.entry_state, a.entry_zone_id, a.entry_country_id, c.customers_telephone, c.customers_fax, c.customers_newsletter, c.customers_default_address_id from " . TABLE_CUSTOMERS . " c left join " . TABLE_ADDRESS_BOOK . " a on c.customers_default_address_id = a.address_book_id where a.customers_id = c.customers_id and c.customers_id = '" . (int)$HTTP_GET_VARS['cID'] . "'");
        $customers_query = tep_db_query("select c.customers_id, c.customers_gender, c.customers_firstname, c.customers_lastname, c.customers_dob, c.customers_email_address, a.entry_firstname, a.entry_lastname, a.entry_company, a.entry_street_address, a.entry_suburb, a.entry_postcode, a.entry_city, a.entry_state, a.entry_zone_id, a.entry_country_id, c.customers_telephone, c.customers_fax, c.customers_newsletter, c.customers_default_address_id from " . TABLE_CUSTOMERS . " c left join " . TABLE_ADDRESS_BOOK . " a on a.address_book_id = " . (isset($HTTP_GET_VARS['add_id']) ? (int)$HTTP_GET_VARS['add_id'] : 'c.customers_default_address_id') . " where a.customers_id = c.customers_id and c.customers_id = '" . (int)$HTTP_GET_VARS['cID'] . "'");
        $customers = tep_db_fetch_array($customers_query);
        $cInfo = new objectInfo($customers);
...
?>
      ...
      <tr>
        <td><table>
          <tr>
            <td class="formAreaTitle"><form>
<?php
$afl = array();
foreach ($aid_list as $a) {
  $afl[] = array('id' => tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('add_id')) . 'add_id=' . $a), 'text' => $a);
}
 echo SELECT_ADDRESS . tep_draw_pull_down_menu('add_id', $afl, tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('add_id')) . 'add_id=' . (isset($HTTP_GET_VARS['add_id']) ? $HTTP_GET_VARS['add_id'] : $cInfo->customers_default_address_id)), 'ONCHANGE="location = this.options[this.selectedIndex].value;"'); ?>
        </form>
            </td>
            <td class="smallText">
<?php
...
<?php
  if ($error == true) {
    ...
    echo tep_draw_input_field('customers_firstname', $cInfo->customers_firstname, 'maxlength="32"', ENTRY_FIRST_NAME_MIN_LENGTH > 0);
...
?></td>
      ...
      <tr>
        ...
        <!-- Admin edit any customer address begin //-->
        <td align="right" class="main"><?php echo tep_image_submit('button_update.gif', IMAGE_UPDATE) . ' <a href="' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('action', 'add_id'))) .'">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>'; ?></td>
...
// Admin edit any customer address begin
    case 'deladdress':
      $heading[] = array('text' => '<b>' . DELETE_ADDRESS . '</b>');
      if (isset($HTTP_GET_VARS['add_id']) && ($HTTP_GET_VARS['add_id'] != $cInfo->customers_default_address_id)) { // if not default address
        $contents = array('form' => tep_draw_form('customersaddress', FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID', 'action','add_id')) . 'cID=' . $cInfo->customers_id . '&action=confirmaddressdelete&add_id=' . $HTTP_GET_VARS['add_id']));
        $contents[] = array('text' => TEXT_DELETE_ADDRESS_INTRO . '<br><br><b>' . $cInfo->customers_firstname . ' ' . $cInfo->customers_lastname . '</b><br><br>' . tep_address_label($cInfo->customers_id, $HTTP_GET_VARS['add_id'], true, '', '<br>'));
        $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.gif', IMAGE_DELETE) . ' <a href="' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=edit') . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
      } else {
        $contents[] = array('text' => '<b>' . TEXT_DELETE_ADDRESS_ERROR . '<b><br>');
        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=edit') . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
      }
      break;
// Admin edit any customer address end
