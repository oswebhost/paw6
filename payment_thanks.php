...
$concatFields = $sk_acct_id . $_GET['transaction_id'] . strtoupper(md5($skrill_secret));
$concatFields = strtoupper(md5($concatFields));

$trans_id = $_GET['transaction_id'];
$msid     = strtoupper($_GET['msid']);




// first check in db if transaction_id is valid payment before showing thank you page.
...
echo "<div style='border:1px solid #3366FF; padding:20px;margin:5px;background:#F2F2FF;font-size:14px;line-height:180%'>";
   ...
   //if ($concatFields == strtoupper($_GET['msid']) ){
	...
	if ( strlen($_GET['transaction_id'])>3 ){
