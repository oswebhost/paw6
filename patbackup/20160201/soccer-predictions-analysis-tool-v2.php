...
//echo $onlyAsl;
 ...
 $qry = "SELECT f.hodddif, f.aodddif, f.dodddif, concat(f.hgoal,f.agoal) as asl2show, f.prvalue, c.dcr_ht, c.dcr_at, c.dcr_av,c.dcr_dif,abs(c.dcr_dif) as dcrdif,f.`div`, f.hteam,f.ateam,f.match_time,f.hgoal,f.agoal,f.h_s,f.a_s,f.gotit,f.mvalue,f.mid,f.pawrank, date_format(f.match_date,'%d-%b-%y') as mdate,
    ...
    case 1: //1x2 betting
       switch ($_GET['CALL'])
       {
			...
			case 8:
                switch ($_GET['CALLPARAM'])
                {
                    case 1: $callpar2 = "  hswin between 0 and ". $odd_max_diff . " or aswin between 0 and " . $odd_max_diff  ; break;
                    case 2: $callpar2 = "  hswin > ". $odd_max_diff . " or aswin > " . $odd_max_diff  ; break;
                }
                break;
      ...
      &nbsp;&nbsp; <font style='font-size:12px;'><b><?php echo find_week_dates($season, $weekno, $db);?></b></font>  
       ...
       <tr>
          ...
          <td class='rtd'><b><font size="2" color="#0000FF">Call Type</font></b></td>
    		  ...
    		  <select size="1" name="CALL" class="text" style="width:140px;padding:3px;" onchange="this.form.submit();">
               ...
               <?}?>
                 ...
                 <? if ($_GET['BETTING']==3 or $_GET['BETTING']==6){ ?>
        		   <option value="1" <?echo selected($_GET['CALL'],'1')?>>Home Win Calls (PaW)</option> 
                   <option value="2" <?echo selected($_GET['CALL'],'2')?>>Away Win Calls (PaW)</option>
               <?}?>
            ...
            <td><input type='text' style='width:40px;text-align:center;' name='min_odd' value='<?php echo num($_GET['min_odd'],2)?>'/> Min
              ...
              <select size="1" name="ASL2GET" class="text" style="width:82px;padding:3px;">
		  ...
		  <tr>
              ...
              <td colspan='1' class='rtd'><b><font size="2" color="#0000FF">Call Parameters</font></b></td>
              <td>
              ...
              <?} ?>	 
               ...
               <td colspan='2' class='rtd padd' style="padding-right: 20px;">
...
<tr bgcolor="#d3ebab">
 ...
 <? if ($_GET['BETTING']=="6"){?>
      <td class='ctd' rowspan="2"><img src="images/tbcap/h-cap.gif" border='0' alt=''/></td>
      <td class='ctd  dark' rowspan="2"><img src="images/tbcap/odds.gif" border='0' alt=''/></td>
   <?}?>
            ...
            if ($_GET['CALL']<5){
				...
				if ($row['mvalue']>0){
						switch ($_GET['CALL']){
						  ...
						  case 6: $win_odds+= ($row['h_s']>$row['a_s']?  $row['h_odd'] : 0) ;
								$asl_class = ($row['h_s']>$row['a_s']?  " gotrt" : "") ;
								$ngot += ($row['h_s']>$row['a_s']? 1 : 0) ;
								
								break;
						  case 7: $win_odds+= ($row['a_s']>$row['h_s']?  $row['a_odd'] : 0) ;
								$asl_class = ($row['a_s']>$row['h_s']?  " gotrt" : "") ;
								$ngot += ($row['a_s']>$row['h_s']? 1 : 0) ;
								
								break;
						  case 8: $win_odds+= ($row['h_s']==$row['a_s']?  $row['d_odd'] : 0) ;
								$asl_class = ($row['h_s']==$row['a_s']?  " gotrt" : "") ;
								$ngot += ($row['h_s']==$row['a_s']? 1 : 0) ;
								break;
						}
...
<tr <?echo rowcol($number);?> class='row'>
   ...
   <? } ?>
    ...
    <? if ($_GET['BETTING']==1){?>
        <td class="ctd <?echo (($_GET["BETTING"]=='1' and $_GET['CALL']==1) ?  $asl_class2: "")?>" <?echo ($_GET["SORTBY"]=='10'? "bgcolor='#D3EBAB'": "");?>><a class='md' href="javascript:tell('full_odds.php?id=<?echo $row['mid'];?>&db=<?echo $db;?>')"><?echo ($row["h_odd"]); ?></a></td>
        <td class="ctd <?echo (($_GET["BETTING"]=='1' and $_GET['CALL']==3) ?  $asl_class2: "")?>" <?echo ($_GET["SORTBY"]=='12'? "bgcolor='#D3EBAB'": "");?>><a class='md' href="javascript:tell('full_odds.php?id=<?echo $row['mid'];?>&db=<?echo $db;?>')"><?echo ($row["d_odd"]); ?></a></td>
        <td class="ctd <?echo (($_GET["BETTING"]=='1' and $_GET['CALL']==2) ?  $asl_class2: "")?>" <?echo ($_GET["SORTBY"]=='11'? "bgcolor='#D3EBAB'": "");?>><a class='md' href="javascript:tell('full_odds.php?id=<?echo $row['mid'];?>&db=<?echo $db;?>')"><?echo ($row["a_odd"]); ?></a></td>
    <?}?>
