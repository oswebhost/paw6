...
$query1 = "SELECT *, date_format(match_date,'%d-%b-%y') as mdate FROM fixtures WHERE hteam=\"$TEAM\" and season='$sea' and `div`='$home_div' ORDER BY match_date";
...
$query1 = "SELECT *, date_format(match_date,'%d-%b-%y') as mdate FROM fixtures WHERE ateam=\"$TEAM\" and season='$sea' and `div`='$away_div' ORDER BY match_date";
if ($db=='eu'){
    $temp = $eu->prepare($query1) ;
}else{
    $temp = $sa->prepare($query1);
...
<?php if ($loc =="Home" ): ?>
	  ...
	  <?php if ($hrank=="rel"): echo "<i> [Relegated] </i>"; endif;
