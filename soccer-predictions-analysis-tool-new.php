...
$PAGE_TITLE = "Soccer Selections Analysis Tool (HandicapPAT)"  ;
...
switch ($_GET['CALLPARAM'])
  {
      case 1: $__CALLPARAM = " ";  break;
      case 2: $__CALLPARAM = " and (o.ht_hcap in (0.50, 1.00, 1.50, 2.00, 2.50, 3.00) or o.at_hcap in (0.50, 1.00, 1.50, 2.00, 2.50, 3.00)) "; break;
      case 3: $__CALLPARAM = " and (o.ht_hcap in (0.25, 0.75, 1.25, 1.75, 2.25, 2.75) or o.at_hcap in (0.25, 0.75, 1.25, 1.75, 2.25, 2.75)) "; break;
  }


if ($_GET['CALL']=="1"){
	$query1 = " o.at_hcap>0 and f.weekno='$weekno'  and f.season='$season' and f.`div`=r.`div` and f.weekno=o.weekno and 
				f.season=o.season and f.mid = o.matchno ";

}elseif ($_GET['CALL']=="2"){

  $query1 = " o.ht_hcap>0 and f.weekno='$weekno'  and f.season='$season' and f.`div`=r.`div` and f.weekno=o.weekno and 
        f.season=o.season and f.mid = o.matchno ";

}elseif($_GET['CALL']=="3"){

  $query1 = " o.ht_hcap=o.at_hcap and f.weekno='$weekno'  and f.season='$season' and f.`div`=r.`div` and f.weekno=o.weekno and 
        f.season=o.season and f.mid = o.matchno ";


}elseif($_GET['CALL']=="4"){ // home strong / win win

	$query1 = " o.at_hcap>0 and o.ht_odd<o.at_odd and f.weekno='$weekno'  and f.season='$season' and f.`div`=r.`div` and f.weekno=o.weekno and 
        f.season=o.season and f.mid = o.matchno ";

}elseif($_GET['CALL']=="5"){ // home strong / away win
	
	$query1 = "o.at_hcap>0 and o.at_odd<o.ht_odd and f.weekno='$weekno'  and f.season='$season' and f.`div`=r.`div` and f.weekno=o.weekno and 
        f.season=o.season and f.mid = o.matchno ";
  
}elseif($_GET['CALL']=="6"){ // away strong / away win
	
	$query1 = " o.ht_hcap>0 and o.at_odd<o.ht_odd and f.weekno='$weekno'  and f.season='$season' and f.`div`=r.`div` and f.weekno=o.weekno and 
        f.season=o.season and f.mid = o.matchno ";
  
}elseif($_GET['CALL']=="7"){ // away strong / win win
	
	$query1 = " o.ht_hcap>0 and o.ht_odd<o.at_odd and f.weekno='$weekno'  and f.season='$season' and f.`div`=r.`div` and f.weekno=o.weekno and 
        f.season=o.season and f.mid = o.matchno ";

}elseif($_GET['CALL']=="8"){ // Equal strong / home win	
	
	$query1 = " o.ht_hcap=o.at_hcap and o.ht_odd<o.at_odd and f.weekno='$weekno'  and f.season='$season' and f.`div`=r.`div` and f.weekno=o.weekno and 
        f.season=o.season and f.mid = o.matchno ";

}elseif($_GET['CALL']=="9"){ // Equal strong / away win 
 
	$query1 = " o.at_odd<o.ht_odd and f.weekno='$weekno'  and f.season='$season' 
		and f.`div`=r.`div` and f.weekno=o.weekno and 
        f.season=o.season and f.mid = o.matchno";

}
  // $_divs $filteron $period ORDER BY o.ht_odd,o.at_odd
 

 $qry = "SELECT f.hodddif, f.aodddif, f.dodddif, concat(f.hgoal,f.agoal) as asl2show, f.prvalue, c.dcr_ht, c.dcr_at, c.dcr_av,c.dcr_dif,
		abs(c.dcr_dif) as dcrdif, f.`div`, f.hteam,f.ateam,f.match_time,f.hgoal,f.agoal,f.h_s,f.a_s,f.gotit,f.mvalue,f.mid,f.pawrank, date_format(f.match_date,'%d-%b-%y') as mdate, f.hwinpb, f.drawpb, f.awinpb, f.h_odd, f.d_odd, f.a_odd, r.ptr_ht, r.ptr_at, 
		r.ptr_av,r.ptr_dif, abs(r.ptr_dif) as ptrdif,(f.hgoal-f.agoal) as goaldif, (f.hgoal+f.agoal) as goalsum, ((f.drawpb*0.5) + f.hwinpb /(f.hwinpb+f.awinpb)*100) as x1probs, ((f.drawpb*0.5) + f.awinpb /(f.hwinpb+f.awinpb)*100) as x2probs, g.gpr_ht, g.gpr_at, g.gpr_av, a.air_ht, a.air_at,a.air_av,a.air_dif, o.ht_odd, o.at_odd, o.ht_hcap, o.at_hcap FROM cur_reb_dcr c, fixtures f, cur_reb_ptr r, ahcap_odds o, cur_reb g, cur_reb_air a   WHERE c.weekno='$weekno' and c.season='$season' and 
...
switch ($_GET['PROPTION'])
  ...
  { 
    ...
    case 6: $call = " and f.hodddif >= $odd_max_diff "; $row3cap = "Home Win Calls (Bookies)"; break;
    case 7: $call = " and f.aodddif >= $odd_max_diff "; $row3cap = "Away Win Calls (Bookies)"; break;
    case 8: $call = " and ( (f.aodddif>0 and f.aodddif <= $odd_max_diff) or (f.hodddif>0 and f.hodddif <= $odd_max_diff) ) "; $row3cap = "Draw Calls (Bookies)"; break;
  }
...
$query  = $qry . $query1 . $proption . $onlyAsl . $_divs . $period . $call .  $filter . $__CALLPARAM .  $callpar . $ordered_by .  $limited ;
...
$xx = "CREATE TEMPORARY TABLE " . $filename . " (".$query.")";
...
<meta name="title" content="Soccer Predictions Analysis Tool (HandicapPAT) <?echo $page_title ?>" />
...
<body>
<?php  page_header("Soccer Predictions Analysis Tool (HandicapPAT)"); $page_title ="Soccer Predictions Analysis Tool (HandicapPAT) "; ?>
...
if ($db=='sa'){
  $pat_url = "db=eu&season=2014-2015";
   ...
   $pat_url = "db=sa&season=2014&DIV=0&BETTING=1&SORTBY=13&ordered=1&PERIOD=1&CALL=3&min_odd=0.00&max_odd=0.00&limitedto=6&PROPTION=1&CALLPARAM=0&B1=View+Data&weekno=" .$wk_to_show ;
}   
...
<table border="0" cellpadding="2" cellspacing="0" style="border:1px solid #ccc;margin:auto auto;background:#E1EFFD;" bordercolor="#f4f4f4" width="1000">
		...
		<tr>
		    ...
		    <select size="1" name="season" class="text" style='padding:3px;width:140px;' onChange="this.form.submit();">
			  <? 
				  ...
				  $sqry = "SELECT distinct(season) as season from fixtures order by season desc limit 2" ;
				  if ($db=='eu'){
      ...
      &nbsp;&nbsp; <font style='font-size:12px;'><b><?php echo find_week_dates($season, $weekno, $db);?></b></font>  
       ...
       <tr>
            ...
            <td>
                <select size="1" name="BETTING" class="text" style="width:140px;padding:3px;" onchange="this.form.submit();">
                   <option value="6" <?echo selected($_GET['BETTING'],'6')?>>Asian Handicap Betting</option> 
      		  </select>
       ...
       <tr>
          ...
          <td class='rtd'><b><font size="2" color="#0000FF">Bookie's Perception</font></b></td>
              <td>
				  <select size="1" name="CALL" class="text" style="width:140px;padding:3px;" onchange="this.form.submit();">
					<option value="1" <? if ($_GET['CALL']==1) echo 'selected';?>>Home Strong</option>
					<option value="2" <? if ($_GET['CALL']==2) echo 'selected';?>>Away Strong </option>
					<option value="3" <? if ($_GET['CALL']==3) echo 'selected';?>>Equal Strength</option>
					<option value="4" <? if ($_GET['CALL']==4) echo 'selected';?>>Home Strong / Home Win</option>
					<option value="5" <? if ($_GET['CALL']==5) echo 'selected';?>>Home Strong / Away Win</option>
					<option value="6" <? if ($_GET['CALL']==6) echo 'selected';?>>Away Strong / Away Win</option>
					<option value="7" <? if ($_GET['CALL']==7) echo 'selected';?>>Away Strong / Home Win</option>
					<option value="8" <? if ($_GET['CALL']==8) echo 'selected';?>>Equal Strength / Home Win</option>
					<option value="9" <? if ($_GET['CALL']==9) echo 'selected';?>>Equal Strength / Away Win</option>
				  </select>
    		</td>
            ...
            <td><input type='text' style='width:40px;text-align:center;' name='min_odd' value='<?php echo num($_GET['min_odd'],2)?>'/> Min
              ...
              <? if ($_GET['BETTING']<>6){ ?>	
                 ...
                 <? if ($_GET['BETTING']==1 and $_GET['CALL']>5 and $_GET['CALL']<8){ ?>	
				    ...
				    <option value="1" <? if ($_GET['CALLPARAM']==1) echo 'selected';?>>All Matches</option>
					<option value="2" <? if ($_GET['CALLPARAM']==2) echo 'selected';?>>Full/Half No. Handicap</option>
					<option value="3" <? if ($_GET['CALLPARAM']==3) echo 'selected';?>>Quarter Handicap</option>
			 ...
			 <?}else{ ?>
					...
					<option value="1" <?echo selected($_GET['CALLPARAM'],'1')?>>All Matches</option>
					<option value="2" <?echo selected($_GET['CALLPARAM'],'2')?>>Full/Half No. Handicap</option>
					<option value="3" <?echo selected($_GET['CALLPARAM'],'3')?>>Quarter Handicap</option>
		   ...
		   <?php if ($_GET['BETTING']==1 and $_GET['CALL']==1){ ?>
			...
			<td>	
				<select size="1" name="CALLAS" class="text" style="width:140px;padding:3px;" >
                   ...
                   <option value="2" <?echo selected($_GET['CALLAS'],'2')?>>Away Calls (PaW) </option>
                   <option value="3" <?echo selected($_GET['CALLAS'],'3')?>>Draw Calls (PaW)</option>
				</select>
			</td>
			<?php } ?>
...
function revCall($no,$secondPara=1){
	$rvcall ="";
	if ($secondPara== 0){

		switch ($no)
		{	
			case 1: $rvcall = "Home Win Calls (PaW)"; break;
			case 2: $rvcall = "Away Win Calls (PaW)"; break;
			case 3: $rvcall = "Draw Calls (PaW)"; break;
		}
		...
		switch ($no)
		{
			case 1: $rvcall = "Home Win Calls (Bookie)"; break;
			case 2: $rvcall = "Away Win Calls (Bookie)"; break;
			case 3: $rvcall = "Draw Calls (Bookie)"; break;
			case 4: $rvcall = "PaW Calls"; break;
		}
	}
	...
	if ($_GET['CALLAS']> 0){
		$row3cap .= "&nbsp;<font color='red'>(Reversed - " . revCall($_GET['CALLAS']) . ")</font>";
	}
	
week_box_new_3rows("Soccer Predictions Analysis Tool (HandicapPAT)", $weekno, $wdate, $season,$row3cap . "<br><font size='1' color='#000000'>$_prerid</font>",900); ?>
...
<tr bgcolor="#d3ebab">
  ...
  <td class='ctd' width="15%"   rowspan="2">
...
<?  
    ...
    while ($row = $temp->fetch()) {
      ...
      switch ($_GET['BETTING']){
		...
		case 6:
			 $captions = "For Single Asian Handicap Betting";
...
<tr <?echo rowcol($number);?> class='row'>
    ...
    <td class="ctd padd"><?echo $number; ?></td>
     ...
     <td class='padd'><?echo $row['hteam'] . printv(' v ') . $row['ateam'] . $my_value;?>
     </td>
   ...
   <? } ?>
    ...
    <? if ($_GET['BETTING']==1 and $_GET['CALLAS']==0){?>
    ...
    <? if ($_GET['BETTING']==2){?>
        <td class="ctd <?echo (($_GET["BETTING"]=='2' and $_GET['CALL']==1) ?  $asl_class2: "")?> " <?echo ($_GET["SORTBY"]=='20'? "bgcolor='#D3EBAB'": "");?> ><a class='md' href="javascript:tell('full_odds.php?id=<?echo $row['mid'];?>&db=<?echo $db;?>')"><?echo ($row["hw_x"]); ?></a></td>
        <td class="ctd <?echo (($_GET["BETTING"]=='2' and $_GET['CALL']==2) ?  $asl_class2: "")?>" <?echo ($_GET["SORTBY"]=='21'? "bgcolor='#D3EBAB'": "");?>><a class='md' href="javascript:tell('full_odds.php?id=<?echo $row['mid'];?>&db=<?echo $db;?>')"><?echo ($row["aw_x"]); ?></a></td>
    ...
    <? if ($_GET['BETTING']==3){?>
        <td class="ctd <?echo (($_GET["BETTING"]=='3' and $_GET['CALL']==1) ?  $asl_class2: "")?>" <?echo ($_GET["SORTBY"]=='30'? "bgcolor='#D3EBAB'": "");?>><a class='md' href="javascript:tell('full_odds.php?id=<?echo $row['mid'];?>&db=<?echo $db;?>')"><?echo ($row["hw_odd"]); ?></a></td>
        <td class="ctd <?echo (($_GET["BETTING"]=='3' and $_GET['CALL']==2) ?  $asl_class2: "")?>" <?echo ($_GET["SORTBY"]=='31'? "bgcolor='#D3EBAB'": "");?>><a class='md' href="javascript:tell('full_odds.php?id=<?echo $row['mid'];?>&db=<?echo $db;?>')"><?echo ($row["aw_odd"]); ?></a></td>
    <?}?>
...
<tr bgcolor="#f4f4f4">
    ...
    <td colspan="5" class="rtd padd bot">Postponed/Void Matches&nbsp;</td>
  ...
  <tr bgcolor="#f4f4f4">
    ...
    <?php }?>
    <td colspan="5" class="ctd padd bot">&nbsp;<?echo $captions; ?>  </td>
  </tr>  
<?}?>
