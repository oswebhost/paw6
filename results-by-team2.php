...
if ($loc =="Home" ){
	$TEAM = $_SESSION['home'] ; $cap="<font color='red'>" . strtoupper($_SESSION[home]) ."</font>" ;
}else{
	$TEAM = $_SESSION['away']; $cap="<font color='blue'>". strtoupper($_SESSION[away]). "</font>" ;  
}
...
$query1 = "SELECT *, date_format(match_date,'%d-%b-%y') as mdate FROM fixtures WHERE ateam=\"$TEAM\" and season='$sea' and `div`='$home_div' ORDER BY match_date";
...
$query1 = "SELECT *, date_format(match_date,'%d-%b-%y') as mdate FROM fixtures WHERE hteam=\"$TEAM\" and season='$sea' and `div`='$away_div' ORDER BY match_date";
...
while ($row = $temp->fetch()){
	...
	if ($row['gotit']=='1'){
		...
		if ($act_goal==$asl_goal){
			$data1.= "<font style='font-size:12px;'><b>". trim($row["h_s"]) ."-" . trim($row["a_s"]) . "</font></b>"; 
		}else{
...
<?php 
	...
	if ($loc =="Home" ){ 
?>
	  <center> <B>When Away: <?php echo $cap?></B>
	  <?php 
...
<?php }else{ ?>
	  ...
	  <center> <B>When Home: <?php echo $cap?></B>
	  <?php 
