...
elseif (!isset($_SESSION['userid']) and ($weekno==cur_week($db))) :
 ...
 $qry = "SELECT c.*, (c.gpr_dif) as gprdif,f.match_time,f.hgoal,f.agoal,f.h_s,f.a_s,f.gotit,
 date_format(f.match_date,'%d-%b-%y') as mdate2 FROM cur_reb c, fixtures f WHERE c.weekno='$weekno' 
 and c.season='$season' and c.matchno=f.mid and f.season=c.season and f.weekno=c.weekno" ;
  ...
  switch ($_GET['SORTBY'])
  {
    case 1: $ordered_by .= " ORDER BY c.gpr_ht desc "; break;
    case 2: $ordered_by .= " ORDER BY c.gpr_at desc " ;  break;
    case 3: $ordered_by .= " ORDER BY gprdif desc";  break;
    case 4: $ordered_by .= " ORDER BY c.gpr_av desc"; break;
  }
...
if (isset($_GET['db'])){
  $page_title = "Current 1X2 Reliabilities " . s_title($db) . " Season $season Week $weekno";
}else{
  $page_title = "Current 1X2 Reliabilities";
}
...
require_once("header.ini.php");
 ...
 page_header("Current 1X2 Reliabilities" ); 
?>
	  ...
	  <form method="get" action="<?php echo $PHP_SELEF;?>">
	    ...
	    <select size="1" name="season" class="text" style="width:100px;">
          ...
          <tr bgcolor="#f4f4f4">
    	    ...
    	    <tr bgcolor="#f4f4f4">	 
    		  ...
    		  <select size="1" name="SORTBY" class="text" style="width:150px;">
        		      <option value="1" <?php echo selected($_GET['SORTBY'],'1')?>>Home Team Reliability</option> 
                   <option value="2" <?php echo selected($_GET['SORTBY'],'2')?>>Away Team Reliability</option>
                   <option value="4" <?php echo selected($_GET['SORTBY'],'4')?>>Average Reliabilities</option>
				   <option value="3" <?php echo selected($_GET['SORTBY'],'3')?>>Diff in Reliabilities</option>
    		  </select>
    		  </td>
		  ...
		  <tr bgcolor="#f4f4f4">
		      ...
		      <input type="submit" value="View Data" name="B1" class="bt" style="padding:3px;"/>
...
<tr bgcolor="#d3ebab">
  ...
  <td width="8%" class='ctd'><img src="images/tbcap/dif.gif" border='0' alt=''/></td>
</tr>
...
<tr <?php echo rowcol($number);?>>
    ...
    <td class="ctd" <?php echo ($_GET["SORTBY"]=='1'? "bgcolor='#D3EBAB'": "");?>><?php echo ($row["gpr_ht"]); ?></td>
    <td class="ctd" <?php echo ($_GET["SORTBY"]=='2'? "bgcolor='#D3EBAB'": "");?>><?php echo ($row["gpr_at"]); ?></td>
    <td class="ctd" <?php echo ($_GET["SORTBY"]=='4'? "bgcolor='#D3EBAB'": "");?>><?php echo ($row["gpr_av"]); ?></td>
    <td class="ctd" <?php echo ($_GET["SORTBY"]=='3'? "bgcolor='#D3EBAB'": "");?>><?php echo ($row["gprdif"]); ?></td>
...
<div style="padding-bottom:8px;padding-left:10px;font-size:11px;" align="left">

<dl style='padding:0;'>
	<dt><b>Home Team Reliability Figure</b>
	<dd>The Home Team figure is the accuracy of the Program's ability to predict the correct result for the Home Team when it is playing at Home.<BR><BR>
	<dt><b>Away Team Reliability Figure</b>
	<dd>The Away Team figure is the accuracy of the Program's ability to predict the correct result for the Away Team when it is playing Away.<BR><BR>
	<dt><b>Difference in Reliabilities</b>
	<dd>Where the Difference is a negative figure it indicates that the Program is more reliable for the Away Team than for the Home Team, otherwise the Home Team has the greater reliability.
</dl>
