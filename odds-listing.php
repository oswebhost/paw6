...
?>
<div class="report_blue_heading" s<div class="report_blue_heading" style="float:left;width: 320px;"><?php echo site($db);?></div>
	...
	<tr>
      ...
      <?php
        ...
        for ($_i=0; $_i<count($arry_div); $_i++){ ?>
            ...
            <option value="<?php echo $arry_div[$_i];?>" <?php echo selected($_GET['div_value'], $arry_div[$_i]);?>><?php echo divname($arry_div[$_i]); ?></option>
      ...
      <td class='ltd'><b><font size="2" color="#0000FF">Sort On</font></b><br/>
         ...
         <select size="1" name="SORTBY" class="text" style="width:160px;padding:3px;">
          ...
          <optgroup label="Probabilities">    
            <option value="7" <?php echo selected($_GET['SORTBY'],'7')?>>Home Wins Probabilities</option>
          </optgroup>    
          
          <option value="15" <?php echo selected($_GET['SORTBY'],'15')?>>Match Date</option>
...
?>
	...
	<tr <?php echo rowcol($number);?>>
	    ...
	    <td class="ctd" <?php echo ($_GET["SORTBY"]=='15'? "bgcolor='#D3EBAB'": "");?>><a class='md2' <?php echo $ffh;?> target='_blank' href='team-performance-chart.php?id=<?php echo $matchno;?>&site=<?php echo $db;?>'><?php echo $row['mdate'] . " " . substr($row['match_time'],0,5); ?></a></td>
    ...
    <td <?php echo ($_GET["SORTBY"]=='7'? "bgcolor='#D3EBAB'": "");?> class='ctd' ><?php echo num2($row['hwinpb']);?></td>
