       ...
       <tr>
            ...
            <td class='rtd'><b><font size="2" color="#0000FF">Call Type</font></b></td>
    		  ...
    		  <select size="1" name="CALL" class="text" style="width:140px;padding:3px;">
    	        <option value="1" <? if ($_GET['CALL']==1) echo 'selected';?>>Bookie's Home Win Calls</option>
                <option value="2" <? if ($_GET['CALL']==2) echo 'selected';?>>Bookie's Away Win Calls</option>
                <option value="3" <? if ($_GET['CALL']==3) echo 'selected';?>>Bookie's Draw Calls</option>
        	  </select>
    		</td>
...
?>
    ...
    <?php if ($weekno<>cur_week($db)){ ?>
         <td class="ctd <?echo $asl_class . $pr;?>" <?echo ($_GET["SORTBY"]=='7' ? "bgcolor='#D3EBAB'": "");?>><?php echo $row['hgoal'] . dash() . $row['agoal']; ?></td>
         <td class="ctd <?echo  $asl_class . $pr;?>">
...
?>
  ...
  <tr <?php echo rowcol($number);?>>
    ...
    <?php if ($weekno<>cur_week($db)){ ?>
         <td class="ctd <?echo $asl_class . $pr;?>"><?php echo $row['hgoal'] . dash() . $row['agoal']; ?></td>
         <td class="ctd <?echo $asl_class2 . $pr;?>">
          <a class='<?echo $sbar2;?>' href="javascript:tell('full_odds.php?id=<?echo $matchno;?>&db=<?echo $db;?>')">
            <? if (strlen($act)<2){
              echo "Odds";
            }else{
              echo $row['h_s'] . dash() . $row['a_s']; 
...
?>
 ...
 <tr bgcolor="#f4f4f4">
    ...
    <td colspan="1" class="ctd padd bot"><?echo $paw_correct; ?></td>
 </tr> 
