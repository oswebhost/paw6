...
<table border="1" cellpadding="5" cellspacing="0" style="border:1px solid #ccc;margin:auto auto;background:#E1EFFD;" bordercolor="#f4f4f4" width="560">
	...
	<tr>
	  ...
	  <?// echo "select * from week_dates where realwkdate<='$curwk_date' and `season` in ('2014-2015','2013-2014', '2012-2013','2011-2012') order by season desc, weekno desc"; ?>
...
<div style='text-align:center;width:550px; margin:10px auto auto 5px;border:1px solid #23488C;background:#E9EFFF;padding:5px;font-size:13px;line-height:140%'>
   The following shows how our <span class='bb'>FREEBIES</span> on the Home Screen have performed for the previous weeks.&nbsp;&nbsp;Remember, these are <span class='bb' style='color:red;'>NOT</span> <span  style='color:red;'>our</span> <span class='bb' style='color:red;'>PRIME SELECTIONS</span>, <i><span class='bb' style='color:purple;'>but the selection process used each week is exactly the same as for the current week's selections you can see on the Home Screen!</span></i>&nbsp;&nbsp;If they were our Prime Selections, why would anybody bother to register with us?&nbsp;&nbsp;The selections on the Home Screen are there simply so you can click to reveal all the backup data for the predictions shown, in the belief that you will be able to <b><i>convince yourself of the power of our unique</i></b> <span class='bb'>Predict-A-Win software Program!</span>&nbsp;&nbsp;Basically, <i><span class='bb' style='color:purple;'>we wish you to register with us</span></i> so that we will know our product is truly appreciated! 
  ...
  for ($iii=0; $iii<=count($from_divs); $iii++){
   ...
   if (get_match_by_week_div($db, $DIV, $weekno, $sea)>2){
      ...
      <tr <?echo rowcol($number);?>>
          ...
          <td class="ctd" title='<?php echo 'Week No ' . $row['weekno'] ?>'><?echo $row['mdate2']  ; ?></td>
          ...
          <td class="ctd"><a title='<?echo $title . " Odds";?>' href="javascript:tell('full_odds.php?id=<?echo $matchno?>&db=<?echo $db?>&season=<?echo $sea?>')" class='sbar'><?echo show_odd($row["asl_odd"])?></a></td>
