...
$qry = "select season,team,`div`,sum(aw+ad+al) as tp,sum(apoint) as points,sum(aw) as w,sum(ad) as d,sum(al) as l,sum(agf) as gf,sum(aga) as ga,sum((agf)-(aga)) as gd from tabs where `div`='$DIV' and season='$sea' and weekno='$wk' ";
...
<table border="1" cellpadding="3" cellspacing="0" style="border-collapse: collapse;" bordercolor="#000000" width="100%" align="center">
 ...
 <tr>
    <td style='background:#f4f4f4;padding:5px;text-align:center;' colspan="12"><font class='credit'>Away League Standings <font color="purple"><?php echo divname($DIV); ?></font></font></td>
 </tr>
...
while ($row = $temp->fetch()){
    ...
    $last5 = last_five_matches($row['team'], $row['div'],"A",$wk,$db);
...
?>
 ...
 <tr <?echo $rowcol; ?>>
    ...
    <td ><?echo $row['team'];?></td>
    <td class='ctd'><?echo $rank;?></td>
    <td class='ctd'><?echo $row['tp'];?></td>
    <td class='ctd'><?echo $row['w'];?></td>
    <td class='ctd'><?echo $row['d'];?></td>
    <td class='ctd'><?echo $row['l'];?></td>
    <td class='ctd'><?echo $row['gf'];?></td>
    <td class='ctd'><?echo $row['ga'];?></td>
    <td class='ctd'><?echo $row['gd'];?></td>
    <td class='ctd'><?echo $row['points'];?></td>
    <td class='ctd'><?echo $last5;?></td>
 </tr>
