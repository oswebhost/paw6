...
function limited_asscess_message($db){
  return "<font style='font-size:14px;'>This is Week No. <b>". cur_week($db)  ."</b> - you will only be able to see the current week's data if you are <a class='sales' href='loginfree.php'>logged in</a> as a registered member.</font>";  
} 
...
function last_five_matches($team, $div,$HomeAway,$cur_Wk, $db){
    ...
    if ($HomeAway=="H"){
        $qry ="select match_date, hteam, ateam, `div`, h_s, a_s from fixtures where season='$season' and `div`='$div' and hteam=\"$team\" and weekno<'$cur_Wk' and h_s<>'P' order by mid desc limit 5";
        
        $qry = "select q.* from ($qry) q order by q.match_date";
    }elseif ($HomeAway=="A"){
        $qry ="select match_date, hteam, ateam, `div`, h_s, a_s from fixtures where season='$season' and `div`='$div' and ateam=\"$team\" and weekno<'$cur_Wk' and h_s<>'P' order by mid desc limit 5";
        $qry = "select q.* from ($qry) q order by q.match_date";
    }else{
        $qry ="select match_date, hteam, ateam, `div`, h_s, a_s from fixtures where season='$season' and `div`='$div' and (hteam=\"$team\" or ateam=\"$team\") and weekno<'$cur_Wk' and h_s<>'P' order by mid desc limit 5";
        $qry = "select q.* from ($qry) q order by q.match_date";
    }
...
echo <<<END
    <!--END page header-->
    <table border="0" cellspacing="0" style="border-collapse: collapse" width="$w" cellpadding="0">
      <tr>
...
<div align="center">
 <table border="0" style="border-collapse: collapse;margin:auto auto;border:1px solid #ccc;"  width="80%" cellpadding="5" >
   <tr class='td' bgcolor='#cccccc'>
       <td> Other  Teams in <span class="bot"><? echo divname($DIV) ?></span> Division</b> </td>
     </tr>
     <tr>
