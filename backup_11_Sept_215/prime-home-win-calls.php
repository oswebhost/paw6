...
<? page_header("Prime Home Win Calls (Top 6)") ; 
...
<div class="report_blue_heading" style="width: 568px;margin:0 auto 5px auto;"><?echo site($db);?></div>

<?  week_box_new($match, $weekno, $wdate, $season,570) ; ?>
...
<tr <?echo rowcol($number);?>>
    ...
    <td class="ctd"><?echo show_odd($row["homepb"]); ?></td>
    <td class="ctd"><?echo show_odd($row["drawpb"]); ?></td>
    <td class="ctd"><?echo show_odd($row["awaypb"]); ?></td>
    <td class="ctd"><a title='<?echo $title?>' href="javascript:tell('full_odds.php?id=<?echo $matchno?>&db=<?echo $db?>')" class='sbar'><?echo $odd?></a></td>
