<? session_start();
  ...
  $type = selection_type($BET) ;
	if (strlen($sql)==0){
		$calls = divname(2) ;
		$qry = "SELECT *,date_format(match_date,'%d-%b-%y') as m_date FROM fixtures where weekno='$WEEKNO'  and season='$SEASON'  order by `div`,mdate,hteam,ateam";
	}else{
		$calls = divname(2) . "<br/><font style='font-size:14px;color:#000;'>$wtype</font>";
		$qry = "SELECT *,date_format(match_date,'%d-%b-%y') as m_date FROM fixtures where weekno='$WEEKNO' and season='$SEASON' $sql  order by `div`,mdate,hteam,ateam";
...
<!-- startprint -->
<? week_box($wdate,$SEASON, $WEEKNO, $calls ."<br>" .site($db), "98%"); ?>
