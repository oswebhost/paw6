...
<div class="report_blue_heading" style="width: 560px;margin:0 auto 5px auto;">Under/Over Odds All Divisions Combined (bookie)</div>
       ...
       <tr>
            ...
            <td class='rtd'><b><font size="2" color="#0000FF">PaW Call Type</font></b></td>
    		  ...
    		  <select size="1" name="CALL" class="text" style="width:140px;padding:3px;">
                  <option value="1" <? if ($_GET['CALL']==1) echo 'selected';?>>All Matches</option>
                  <option value="2" <? if ($_GET['CALL']==2) echo 'selected';?>>Home - Under Calls</option>
                  <option value="3" <? if ($_GET['CALL']==3) echo 'selected';?>>Home - Over Calls</option>
                  <option value="4" <? if ($_GET['CALL']==4) echo 'selected';?>>Away - Under Calls</option>
                  <option value="5" <? if ($_GET['CALL']==5) echo 'selected';?>>Away - Over Calls</option>
                  <option value="6" <? if ($_GET['CALL']==6) echo 'selected';?>>Draw - Under Calls</option>
                  <option value="7" <? if ($_GET['CALL']==7) echo 'selected';?>>Draw - Over Calls</option>
 		      </select>
    		</td>
        ...
        <select size="1" name="SORTBY" class="text" style="width:150px;padding:3px;">
          <optgroup label="Odds">  
            <option value="10" <?echo selected($_GET['SORTBY'],'10')?>>Under Odds</option>
            <option value="11" <?echo selected($_GET['SORTBY'],'11')?>>Over Odds</option>
          </optgroup>
...
if ($db=="eu"){
 ...
 switch ($_GET['PERIOD'])
	...
	$bookie_t_under=0; $bookie_t_over=0; $bookie_t_under_correct=0; $bookie_t_over_correct=0; 
	...
	for ($week_loop=1; $week_loop<=$mx_week; $week_loop++){
		  ...
		  ?>
			<tr <?php echo rowcol($week_loop);?>>
				...
				<td class='ctd'><?php echo num($bookie_under,0); ?></td>
				<td class='ctd bold'><?php echo num(($bookie_under / ($week_t_under>0? $week_t_under :1)) * 100, 2); ?>%</td>

				<td class='ctd'><?php echo num($week_t_over,0); ?></td>
				<td class='ctd'><?php echo num($bookie_over,0); ?></td>
				<td class='ctd bold'><?php echo num(($bookie_over / ($week_t_over>0? $week_t_over :1)) * 100, 2); ?>%</td>
		...
		<?php
			...
			$paw_t_under_correct += $bookie_under; 
			$paw_t_over_correct += $bookie_over; 
		...
		<tr bgcolor="#f4f4f4">
			...
			<td class='ctd padd bot'><?php echo num( ($paw_t_under_correct/($bookie_t_under>0?$bookie_t_under:1))*100 ,2);?>%</td>
			<td class='ctd padd bot'><?php echo $bookie_t_over;?></td>
			<td class='ctd padd bot'><?php echo $paw_t_over_correct;?></td>
			<td class='ctd padd bot'><?php echo num( ($paw_t_over_correct/($bookie_t_over>0?$bookie_t_over:1))*100 ,2);?>%</td>
		</tr>
